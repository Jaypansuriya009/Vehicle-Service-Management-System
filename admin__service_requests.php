...
// Fetch service requests for different statuses
function fetchData($conn, $status) {
    $stmt = $conn->prepare("SELECT id, vehicle_name, vehicle_model, vehicle_brand, service_date, status FROM service_requests WHERE status = ?");
    $stmt->bind_param("s", $status);
    $stmt->execute();
    return $stmt->get_result();
}

$Pending_requests = fetchData($conn, "Pending");
$rejected_requests = fetchData($conn, "Rejected");
$completed_requests = fetchData($conn, "Completed");
?>
...
<ul class="dropdown-menu w-100" aria-labelledby="dropdownMenuLink">
    ...
    <a href="admin__service_requests.php" class="active"><i class="fas fa-clipboard-list"></i> Service Requests</a>
...
<div>
    ...
    <div class="table-container">
        <h3 class="table-title completed">Pending Service Requests</h3>
                ...
                <tbody>
                    <?php while ($row = $Pending_requests->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                <a href="view_details.php?id=<?php echo $row['id']; ?>" class="btn btn-info btn-sm">
        ...
        <?php else: ?>
            <p class="text-center text-danger">No New service requests found.</p>
        <?php endif; ?>
    </div>

    <!-- Rejected Requests Table -->
    <div class="table-container " style="margin-top: 50px;">
        <h3 class="table-title rejected">Rejected Service Requests</h3>
        ...
        <?php else: ?>
            <p class="text-center text-danger">No rejected service requests found.</p>
        <?php endif; ?>
    </div>

    <div class="table-container">
        <h3 class="table-title cancelled">Completed Service Requests</h3>
        ...
        <?php else: ?>
            <p class="text-center text-danger">No Completed service requests found.</p>
        <?php endif; ?>
    </div>
</div>
</div>
